...
$(call require,mod.mk,HUI_OS)

HUI_INIT_SCRIPT = init-octoprint.sh
...
exit
# Back as root.
cp ${${HUI_OS_VARIANT}_TMP_DIR}/octoprint.service /etc/systemd/system
...
Type=exec
User=${HUI_USER}
ExecStart=/home/${HUI_USER}/OctoPrint/venv/bin/octoprint
...
define stage_${HUI_SOFTWARE}
  printf "%s" "$$OctoPrintInitScript" > $(1)/${HUI_INIT_SCRIPT}; \
  printf "%s" "$$OctoPrintService" > $(1)/octoprint.service
endef

endif

include ${MK_DIR}/${HUI_OS}.mk
...
Defined in mod.mk:
  ...
  HUI_OS = ${HUI_OS}
    Which OS is installed on the user interface board (HUI_OS_BOARD).

Defined in config.mk:

Defined in ${HUI_OS}.mk or a segment it loads:
  OsDeps = ${OsDeps}
    A list of dependencies needed in order to mount an OS image for
    modification.

Defines:
  HUI_INIT_SCRIPT = ${HUI_INIT_SCRIPT}
